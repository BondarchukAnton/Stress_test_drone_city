...
* На каждом дроне на Pi-5: бинарный файл PicoClaw (или оболочковый wrapper) + HTTP-мост (контракт `/healthz`, `/takeoff`, `/photograph`, `/move`) + камера. Контракт HTTP совместим с `bridge/mock.py`.
